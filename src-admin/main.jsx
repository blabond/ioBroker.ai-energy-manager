...
function SettingsTab({
  ...
  useEffect(() => {
    setExpandedPanels((current) => {
      const next = {};
      for (const [key, defaultExpanded] of Object.entries(
...
function UpdateAvailableNotice() {
  return (
    <Alert severity="info" icon={false} className="update-notice">
      <Box className="update-notice-content">
        ...
        <Button variant="contained" size="small" onClick={reloadWithCacheBust}>
          {t("reloadNow")}
        </Button>
      </Box>
    </Alert>
  ...
  return (
    <Stack spacing={1.5}>
      <Box className="plan-timeline" aria-label={t("sixHourPlan")}>
        {groups.map((group) => (
          <Box className="plan-hour-group" key={group.key}>
...
function DatapointAssignmentRow({
  assignment,
  index,
  selectStatePath,
  updateAssignment,
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {renderPowerTypeSelect(assignment, index, updateAssignment)}
      </TableCell>
      <TableCell>
        <TextField
        ...
        {customScriptStatePathWarning(assignment.stateId) ? (
          <Alert className="custom-state-path-warning" severity="error">
      ...
      <TableCell>
        {assignment.required ? (
          <Chip color="primary" size="small" label={t("required")} />
        ) : null}
      </TableCell>
    </TableRow>
  );
...
function groupHasMissingRequiredState(group) {
  return group.rows.some(
    ({ assignment }) =>
      ...
      String(assignment.stateId || "").trim().length === 0,
  );
}

function groupAssignmentSections(group) {
  const controlRows = group.rows.filter(({ assignment }) =>
    isBatteryControlAssignment(assignment),
  );
  ...
  const telemetryRows = group.rows.filter(
    ({ assignment }) => !isBatteryControlAssignment(assignment),
  );
  return [
    {
      key: "telemetry",
...
async function fetchLatestUiAsset() {
  try {
    ...
    const components = Array.isArray(manifest?.exposes)
      ...
      : null;
    const asset = components?.assets?.js?.sync?.find((item) =>
...
async function clearAdapterCacheEntries() {
  ...
  try {
    ...
    await Promise.all(
      keys
        .filter((key) =>
          /ai-energy-manager|ConfigCustomAiEnergyManager|iobroker/i.test(key),
        )
