...
class TelemetryQueue {
    constructor(adapter, apiClient, options = {}) {
        ...
        this.maxQueueItems = Number(options.maxQueueItems || DEFAULT_MAX_QUEUE_ITEMS);
    ...
    async load() {
        try {
            ...
            this.items = [];
            await this.adapter.setError(`Telemetry queue could not be loaded: ${error.message}`);
        }
    }

    async enqueue(payloads) {
        const items = (Array.isArray(payloads) ? payloads : [payloads]).filter(isPlainObject);
        if (items.length === 0) {
            return;
        }
        ...
        if (this.items.length > this.maxQueueItems) {
            ...
            this.adapter.log.warn(`Telemetry queue is full, ${overflow} older entries were discarded.`);
        }
        await this.persist();
    }
    ...
    async persist() {
        await this.adapter.setStateAsync(QUEUE_STATE_ID, JSON.stringify(this.items), true);
        await this.updateQueueLength();
    }

    async updateQueueLength() {
        await this.adapter.setStateAsync('telemetry.queueLength', this.items.length, true);
    }
}
