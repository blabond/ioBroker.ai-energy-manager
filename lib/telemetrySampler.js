...
const TELEMETRY_VERSION = 'adapter-telemetry-2026-05-24';
...
class TelemetrySampler {
    constructor(adapter, datapointManager, options = {}) {
        ...
        this.sendIntervalMs = Number(options.sendIntervalMs || DEFAULT_SEND_INTERVAL_MS);
        this.maxPlausibleMeterPowerW = Number(options.maxPlausibleMeterPowerW || DEFAULT_MAX_PLAUSIBLE_METER_POWER_W);
        this.maxPlausibleGridPowerW = Number(options.maxPlausibleGridPowerW || DEFAULT_MAX_PLAUSIBLE_GRID_POWER_W);
        this.inverterEfficiency = Number(options.inverterEfficiency || DEFAULT_INVERTER_EFFICIENCY);
        this.buffers = {};
        this.pvBuffers = {};
    ...
    async configure(serverConfig) {
        ...
        for (const definition of definitions) {
            ...
            const shouldSubscribe = this.addStateTracker(definition.stateId, definition.key);
            if (shouldSubscribe) {
    ...
    async close() {
        ...
        await Promise.all(stateIds.map(stateId => this.unsubscribeState(stateId)));
    }
    ...
    async collect(serverConfig) {
        ...
        for (const plant of plantsFromConfig(serverConfig)) {
            ...
            assignIfNumber(payload, 'consumption_wh', household.consumptionMeterWh);
            assignIfNumber(payload, 'grid_export_meter_wh', household.gridExportMeterWh);
            assignIfNumber(payload, 'grid_import_wh', household.gridImportWh);
            assignIfNumber(payload, 'grid_export_wh', household.gridExportWh);
            assignIfNumber(payload, 'grid_power_import_wh', household.gridPowerImportWh);
            assignIfNumber(payload, 'grid_power_export_wh', household.gridPowerExportWh);
            ...
            assignIfNumber(
                payload,
                'battery_soc_percent',
                await this.readPlantNumber(plant.id, ['batterySoc', 'storageSystemSoc']),
            );
            assignIfNumber(payload, 'battery_power_w', await this.readPlantPowerW(plant.id, BATTERY_POWER_KEYS));
            assignIfNumber(payload, 'pv_power_w', await this.readPlantPvPowerW(plant));

            if (battery && battery.samples > 0) {
                assignIfNumber(payload, 'battery_output_wh_total', battery.outputWh);
                assignIfNumber(payload, 'battery_household_energy_wh_total', battery.householdWh);
                assignIfNumber(payload, 'battery_input_wh_total', battery.inputWh);
                assignIfNumber(payload, 'battery_grid_charge_wh_total', battery.gridChargeWh);
                assignIfNumber(payload, 'battery_surplus_charge_wh_total', battery.surplusChargeWh);
            }
    ...
    async collectHousehold() {
        const consumption = await this.energyCounterReading('consumptionWh', 'household');
        const gridExport = await this.energyCounterReading('gridExportMeterWh', 'household');
        const wallbox = await this.energyCounterToIntervalWh('wallboxWh', 'household');
        const gridPower = this.collectGridPowerTotals();

        return {
            consumptionMeterWh: consumption.readingWh,
            gridExportMeterWh: gridExport.readingWh,
            wallboxWh: wallbox.wh,
            gridImportWh:
                consumption.deltaWh !== null
                    ? consumption.deltaWh
                    : !consumption.usable && gridPower.samples > 0
                      ? gridPower.importWh
                      : null,
            gridExportWh:
                gridExport.deltaWh !== null
                    ? gridExport.deltaWh
    ...
    async readPlantPvPowerW(plant) {
        ...
        if (Number(plant.battery_capacity_kwh || plant.batteryCapacityKwh || 0) <= 0) {
    ...
    async energyCounterToIntervalWh(key, scopeId) {
        const reading = await this.energyCounterReading(key, scopeId);
        return {
            wh: reading.deltaWh,
    ...
    async energyCounterReading(key, scopeId) {
        ...
        const readingWh = value * factor.factor;
        const wh = this.positiveMeterDeltaWh(assignment.stateId, value, factor.factor, {
    ...
    positiveMeterDeltaWh(stateId, value, factorToWh, options = {}) {
        ...
        const maxDeltaWh = Math.max(1, this.maxPlausibleMeterPowerW * (this.sendIntervalMs / 3600000));
        if (options.allowRawDeltaFallback && convertedDelta > maxDeltaWh && rawDelta <= maxDeltaWh) {
    ...
    async readNumber(stateId) {
        ...
        if (!this.missingLogged.has(stateId)) {
            ...
            this.adapter.log.info(`Datapoint is not readable, telemetry value skipped: ${stateId}`);
        }
        return null;
    }
    ...
    assignmentFor(scopeId, key) {
        ...
        return assignments.find(
            item =>
                item &&
                item.key === key &&
                (item.scopeId === scopeId || (scopeId === 'household' && item.scope === 'household')),
        );
    }
    ...
    powerTrackerDefinitions(serverConfig) {
        ...
        for (const plant of plantsFromConfig(serverConfig)) {
            ...
            if (batteryAssignment && isBatteryCapablePlant(plant)) {
                definitions.push(this.powerTrackerDefinition('battery', batteryAssignment));
            }
    ...
    powerTrackerDefinition(kind, assignment) {
        ...
        const scopeId = assignment.scope === 'household' ? 'household' : String(assignment.scopeId);
        return {
    ...
    removeTracker(key, tracker) {
        ...
        if (keys.size === 0) {
            ...
            this.unsubscribeState(tracker.stateId).catch(error =>
                this.adapter.log.debug(`Datapoint unsubscribe failed: ${tracker.stateId}: ${error.message}`),
            );
        }
    }
    ...
    updatePowerTracker(tracker, value, timestampMs) {
        ...
        tracker.lastTimestampMs = Math.max(timestampMs, tracker.lastTimestampMs || timestampMs);
    }

    integratePowerTracker(tracker, nowMs) {
        if (!Number.isFinite(tracker.lastValueW) || !Number.isFinite(tracker.lastTimestampMs)) {
    ...
    householdContributionWh(tracker, wh) {
        return normalizePowerType(tracker.powerType) === 'AC' ? wh : wh * this.inverterEfficiency;
    }
    ...
    currentGridPowerForClassification(nowMs) {
        ...
        if (!tracker || !Number.isFinite(tracker.lastValueW) || !Number.isFinite(tracker.lastTimestampMs)) {
            return null;
        }
    ...
    isPlausiblePowerW(tracker, valueW) {
        ...
        const limit = tracker.kind === 'grid' ? this.maxPlausibleGridPowerW : this.maxPlausibleMeterPowerW;
...
function normalizePlant(item, index) {
    ...
    return {
        ...item,
        id: String(item.id || item.plantId || item.uuid || item._id || item.key || '').trim(),
        name: String(item.name || item.label || `Plant ${index + 1}`),
    };
}

function isBatteryCapablePlant(plant) {
    if (plant && Object.prototype.hasOwnProperty.call(plant, 'battery_capacity_kwh')) {
        return Number(plant.battery_capacity_kwh || 0) > 0;
    }
    if (plant && Object.prototype.hasOwnProperty.call(plant, 'batteryCapacityKwh')) {
        return Number(plant.batteryCapacityKwh || 0) > 0;
    }
    if (plant && plant.features && Object.prototype.hasOwnProperty.call(plant.features, 'battery')) {
...
function isReadablePowerAssignment(assignment) {
    return (
        assignment && assignment.read !== false && assignment.unit === 'W' && String(assignment.stateId || '').trim()
    );
}
