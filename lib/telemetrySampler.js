...
class TelemetrySampler {
  ...
  async collect(serverConfig) {
    ...
    for (const plant of plantsFromConfig(serverConfig)) {
      ...
      if (battery && battery.samples > 0) {
        ...
        if (battery.inputWh > 0) {
          const gridImportWh = Number(payload.grid_import_wh || 0);
          payload.battery_grid_charge_wh_total =
            gridImportWh > 0 ? battery.inputWh : 0;
          payload.battery_surplus_charge_wh_total =
            gridImportWh > 0 ? 0 : battery.inputWh;
        }
      }
  ...
  applyHouseholdEnergyBalance(payload) {
    const gridImportWh = numberOrZero(payload.grid_import_wh);
    const gridExportWh = numberOrZero(payload.grid_export_wh);
    const batteryHouseholdWh = numberOrZero(
      payload.battery_household_energy_wh_total,
    );
    if (
      ...
      payload.estimated_household_consumption_wh = Math.max(
        0,
        gridImportWh + Math.max(0, batteryHouseholdWh - gridExportWh),
      );
    }
  }
  ...
  integratePowerTracker(tracker, nowMs) {
    ...
    if (tracker.kind === "battery") {
      ...
      if (tracker.lastValueW > 0) {
        buffer.outputWh += wh;
        buffer.householdWh += wh * this.inverterEfficiency;
      } else if (tracker.lastValueW < 0) {
        buffer.inputWh += wh;
      }
    }
...
function emptyBuffer() {
  return { outputWh: 0, householdWh: 0, inputWh: 0, samples: 0 };
}
