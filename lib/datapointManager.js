...
class DatapointManager {
  ...
  configuredAssignments() {
    return Array.isArray(this.adapter.config.datapointAssignments)
    ...
    if (assignments.length > 0) {
      return Object.fromEntries(
        assignments.map((item) => [
          item.mappingKey || item.key,
          String(item.stateId || "").trim(),
        ]),
      );
    }
    return this.adapter.config.datapoints || {};
  }
    ...
    for (const definition of datapoints) {
      ...
      if (!stateId) {
        if (definition.required) {
          errors.push(
            `${definition.label || definition.key}: datapoint missing.`,
          );
        }
        continue;
      }
      ...
      if (!object || object.type !== "state") {
        errors.push(
          `${definition.label || definition.key}: state ${stateId} does not exist.`,
        );
        continue;
      }
      ...
      if (definition.read && common.read === false) {
        ...
        if (!readable && definition.required) {
          errors.push(
            `${definition.label || definition.key}: state ${stateId} is not readable.`,
          );
        } else if (!readable) {
          warnings.push(
      ...
      if (definition.write && common.write !== true) {
        errors.push(
          `${definition.label || definition.key}: state ${stateId} is not writable.`,
        );
      }
      if (!definition.write && common.write === true) {
        warnings.push(
