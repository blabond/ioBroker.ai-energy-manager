...
class StateCollector {
    ...
    async collect(serverConfig) {
        ...
        for (const definition of definitions) {
            ...
            const stateId = this.datapointManager.stateIdFor(definition.mappingKey || definition.key);
            if (!stateId) {
        ...
        for (const setting of plantSettings) {
            ...
            plants[setting.installationId].gridChargingAllowed = setting.gridChargingAllowed;
            plants[setting.installationId].grid_charging_allowed = setting.gridChargingAllowed;
        }
        return {
