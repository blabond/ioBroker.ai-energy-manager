...
const DEMO_ACCOUNT_BACKEND_TOKEN = 'DEMOACCOUNT';
const DEMO_ACCOUNT_TOKEN_PREFIXES = ['DEMOACCOUNT', 'DEMOACC'];
...
function isDemoAccountToken(value) {
    const token = String(value || '')
        .trim()
        .toUpperCase();
    return DEMO_ACCOUNT_TOKEN_PREFIXES.some(prefix => token === prefix || token.startsWith(`${prefix}-`));
}

function normalizeBackendToken(value) {
    const token = assertToken(value);
    return isDemoAccountToken(token) ? DEMO_ACCOUNT_BACKEND_TOKEN : token;
}

function isAuthenticationError(value) {
    return /\b(authentication failed|unauthori[sz]ed|invalid token|token .*rejected|http\s*(401|403))\b/iu.test(
        String(value || ''),
    );
}
...
function validateAdapterConfig(nativeConfig) {
    ...
    return {
        ...
        backendUrl,
        adapterToken,
        sendIntervalSeconds: normalizeInterval(nativeConfig.sendIntervalSeconds, 10, 3600, 60),
        commandPollIntervalSeconds: normalizeInterval(nativeConfig.commandPollIntervalSeconds, 10, 3600, 30),
        minWriteIntervalSeconds: normalizeInterval(nativeConfig.minWriteIntervalSeconds, 0, 3600, 10),
...
module.exports = {
    DEFAULT_BACKEND_URL,
    DEMO_ACCOUNT_BACKEND_TOKEN,
    DEMO_ACCOUNT_TOKEN_PREFIX,
    DEMO_ACCOUNT_TOKEN_PREFIXES,
