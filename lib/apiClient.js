...
const USER_AGENT = `iobroker.ai-energy-manager/${version}`;

class ApiClient {
    constructor(options) {
        this.backendUrl = normalizeBackendUrl(options.backendUrl);
        this.token = assertToken(options.token);
        this.backendToken = normalizeBackendToken(this.token);
    ...
    async request(method, path, body) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(maskToken(message, this.token, this.backendToken));
            }
            return data;
        } catch (error) {
    ...
    setRequestTimeout(callback) {
        ...
        return globalThis.setTimeout(callback, this.timeoutMs);
    }
...
function maskToken(message, ...tokens) {
    return tokens.reduce(
        (masked, token) => (String(token || '') ? masked.split(String(token || '')).join(maskSecret(token)) : masked),
        String(message || ''),
    );
}
