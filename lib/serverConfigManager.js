...
function normalizeServerConfig(serverConfig) {
  ...
  if (serverConfig.success === false) {
    return invalid(
      serverConfig.message || "Backend rejected the configuration.",
    );
  }
  ...
  const features = isPlainObject(serverConfig.features)
  ...
  const requiredDatapoints = isPlainObject(serverConfig.requiredDatapoints)
    ? serverConfig.requiredDatapoints
    : {};
  const datapoints = datapointsForFeatures(enabledFeatures, requiredDatapoints);
  const missingRequiredDefinitions = datapoints
    .filter((item) => item.required && !item.label)
    .map((item) => item.key);
  if (missingRequiredDefinitions.length > 0) {
    return invalid(
      `Incomplete datapoint definitions: ${missingRequiredDefinitions.join(", ")}`,
    );
  }
  ...
  return {
    ...
    unsupportedFeatures,
    datapoints,
    requiredDatapoints: Object.fromEntries(
      datapoints.map((item) => [item.key, item]),
    ),
    controlModes: isPlainObject(serverConfig.controlModes)
      ? serverConfig.controlModes
      : {},
    raw: serverConfig,
    errors: [],
  };
