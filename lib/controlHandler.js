...
class ControlHandler {
  constructor(adapter, datapointManager, apiClient, options = {}) {
    ...
    this.minWriteIntervalMs =
      Number(options.minWriteIntervalSeconds || 0) * 1000;
  ...
  async handleCommand(serverConfig, command) {
    ...
    const definition =
      this.datapointManager.writableDefinitionForTarget?.(
        serverConfig,
        target,
      ) || null;
    const stateId = definition
      ? this.datapointManager.stateIdFor(definition.mappingKey || target) ||
        null
      : null;
    if (!commandId || !target) {
      return this.result(
        false,
        "invalid_command",
        "Command ID or target is missing.",
      );
    }
    if (!stateId) {
      return this.result(
    ...
    if (
      this.minWriteIntervalMs > 0 &&
      now - lastWriteAt < this.minWriteIntervalMs
    ) {
      return this.result(
        false,
        "rate_limited",
...
function commandRequestsGridCharge(definition = {}, command = {}) {
  ...
  if (
    actionCandidates.some((value) =>
      ["gridcharge", "grid_charge", "netzladen"].includes(value),
    )
  ) {
    return true;
  }
  ...
  const gridChargeValue = normalizeCommandText(
    definition.controlValues?.gridCharge,
  );
