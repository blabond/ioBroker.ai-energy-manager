...
function buildDatapointAssignments(
  ...
  for (const [index, plant] of plants.entries()) {
    const definitions = ensureBatteryControlDefinition(
      plant,
      definitionsForPlant(plant, plantDefinitions),
    );
...
function ensureBatteryControlDefinition(plant, definitions) {
  if (
    !plantHasBattery(plant, definitions) ||
    definitions.some((item) => item.key === BATTERY_CONTROL_DEFINITION.key)
  ) {
    return definitions.map((item) =>
      item.key === BATTERY_CONTROL_DEFINITION.key
        ? withBatteryControlMetadata(item, plant)
        : item,
    );
  }
  return [
    ...definitions,
    withBatteryControlMetadata(BATTERY_CONTROL_DEFINITION, plant),
  ];
}
...
function plantHasBattery(plant, definitions) {
  if (plant?.features && typeof plant.features === "object") {
    return (
      ...
      (plant.features.storageSystem !== false &&
        plant.features.storageSystem != null)
    );
  }
  return definitions.some((item) =>
    ["battery", "storageSystem"].includes(String(item.feature || "")),
  );
}
  ...
  if (Array.isArray(existingAssignments)) {
    for (const assignment of existingAssignments) {
      ...
      if (Object.hasOwn(assignment, "gridChargingAllowed")) {
        data.gridChargingAllowed = normalizeBoolean(
          assignment.gridChargingAllowed,
        );
      }
