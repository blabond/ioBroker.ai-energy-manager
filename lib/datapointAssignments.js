...
function buildDatapointAssignments(serverConfig, existingAssignments = [], legacyDatapoints = {}) {
    if (!serverConfig?.valid) {
        return [];
    }
    const previous = previousAssignmentData(existingAssignments, legacyDatapoints);
    const assignments = [];
...
function previousAssignmentData(existingAssignments = [], legacyDatapoints = {}) {
    const previous = {};
