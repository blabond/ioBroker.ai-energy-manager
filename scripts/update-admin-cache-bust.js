...
const components = Array.isArray(manifest.exposes) ? manifest.exposes.find(item => item?.name === 'Components') : null;
const mainAsset = components?.assets?.js?.sync?.find(item => /(^|\/)main-[^/]+\.js$/u.test(String(item || '')));
...
const updatedJsonConfig = jsonConfig.replace(
    /url:\s*(['"])custom\/customComponents\.js(?:\?v=[^'"]*)?\1/u,
...
if (updatedJsonConfig === jsonConfig) {
    if (!jsonConfig.includes(`custom/customComponents.js?v=${cacheKey}`)) {
        throw new Error('Unable to update custom component URL in admin/jsonConfig.json5');
    }
} else {
