...
} = require("./lib/serverConfigManager");
...
class AiEnergyManager extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!config.valid) {
      ...
      this.log.warn(
        `Adapter configuration incomplete: ${config.errors.join(" ")}`,
      );
      return;
    }
    ...
    if (!tokenValid) {
      ...
      await this.resetDashboardStates();
      this.log.info(
        "Adapter is waiting for a valid stored token and Request Config.",
      );
      return;
    }
    if (this.shouldAutoRequestServerConfig(config.adapterToken)) {
      ...
      if (!result.ok) {
        await this.resetDashboardStates();
        this.log.info(
          "Adapter is waiting for a successful server configuration.",
        );
        return;
      }
      await this.applyRuntimeServerConfig(
        result.serverConfig,
    ...
    if (!this.serverConfig?.valid) {
      await this.resetDashboardStates();
      this.log.info(
        "Adapter is waiting for a successful manual Request Config.",
      );
      return;
    }
  ...
  async validateToken() {
    try {
      ...
      if (!valid) {
        await this.setError(
          response.message || "Adapter token was rejected by the backend.",
        );
      }
      return valid;
    } catch (error) {
  ...
  async requestConfigFromBackend(options = {}) {
    const overrideToken =
      typeof options.adapterToken === "string"
        ...
        : "";
    const nativeConfig = overrideToken
      ? { ...this.config, adapterToken: overrideToken }
      : this.config;
    ...
    try {
      ...
      if (shouldPersist) {
        await this.persistServerConfig(
          normalized,
  ...
  async requestDashboardLite(options = {}) {
    ...
    try {
      ...
      const installationId = String(
        options.installationId || this.config.serverConfig?.plantId || "",
      ).trim();
  ...
  async readDashboardLiteState() {
    try {
      ...
      const hasDashboard =
        dashboardLite &&
        Array.isArray(dashboardLite.cards) &&
        dashboardLite.cards.length > 0;
  ...
  async loadStoredServerConfig() {
    try {
      ...
      if (!stored) {
        await this.setStateAsync("info.configValid", false, true);
        await this.setError(
          "No server configuration loaded yet. Please run Request Config in the admin UI.",
        );
        await this.resetDashboardStates();
        return;
      }
      ...
      await this.persistDatapointAssignmentsIfChanged(
        this.config.datapointAssignments,
      );
      ...
      await this.resetDashboardStates();
      await this.setError(
        `Stored server configuration is invalid: ${error.message}`,
      );
    }
  }
  ...
  async persistServerConfig(
    ...
    this.config.serverConfigTokenFingerprint =
      object.native.serverConfigTokenFingerprint;
    this.config.datapointAssignments = datapointAssignments;
  }

  async applyRuntimeServerConfig(
    serverConfig,
    requestedAt = new Date().toISOString(),
  ) {
  ...
  async persistDatapointAssignmentsIfChanged(datapointAssignments = []) {
    ...
    const current = Array.isArray(object.native?.datapointAssignments)
      ? object.native.datapointAssignments
      : [];
    if (
      JSON.stringify(current) ===
      JSON.stringify(
        Array.isArray(datapointAssignments) ? datapointAssignments : [],
      )
    ) {
      return;
    }
  ...
  async publishRuntimeServerConfigStates() {
    await this.setStateAsync(
      ...
      JSON.stringify(this.serverConfig || null),
      true,
    );
    await this.setStateAsync(
      "info.datapointAssignmentsJson",
      JSON.stringify(
        Array.isArray(this.config.datapointAssignments)
          ? this.config.datapointAssignments
          : [],
      ),
      true,
  ...
  async applyServerConfigStatus(serverConfig, requestedAt = "") {
    ...
    await this.setStateAsync(
      ...
      Number(serverConfig.version || 0),
      true,
    );
    await this.setStateAsync(
      "info.serverConfigRevision",
      Number(serverConfig.configRevision || 0),
      true,
    );
    await this.setStateAsync(
  ...
  shouldAutoRequestServerConfig(adapterToken) {
    ...
    return (
      this.config.serverConfigTokenFingerprint !==
      adapterTokenFingerprint(adapterToken)
    );
  }
  ...
  async validateDatapoints() {
    ...
    if (!result.valid) {
      ...
      this.log.warn(
        `Datapoint configuration incomplete: ${result.errors.join(" ")}`,
      );
      return false;
    }
  ...
  async scheduleTelemetrySend() {
    ...
    this.statePayloadTimer = this.setTimeout(async () => {
      ...
      try {
        ...
        this.scheduleTelemetrySend().catch((error) =>
  ...
  async scheduleServerPull() {
    ...
    this.commandTimer = this.setTimeout(async () => {
      ...
      try {
        ...
        this.scheduleServerPull().catch((error) =>
          this.setError(error.message),
        );
      }
    }, delayMs);
  ...
  async pollOffsetSeconds() {
    ...
    if (
      ...
      stored >= 0 &&
      stored <= MAX_POLL_OFFSET_SECONDS &&
      state?.q !== 32
    ) {
      return stored;
    }
  ...
  async pushAdapterMappings() {
    ...
    if (
      mappings.assignments.length === 0 &&
      mappings.plantSettings.length === 0
    ) {
      return;
    }
  ...
  adapterMappingsPayload() {
    const assignments = Array.isArray(this.config.datapointAssignments)
      ? this.config.datapointAssignments
      : [];
    return {
      ...
      assignments: assignments
        .map((assignment) => adapterMappingFromAssignment(assignment))
        .filter(Boolean),
    };
  }
  ...
  async updateDashboardLiteFromBackend() {
    ...
    const decision = await this.apiClient.requestDashboardDecision(
      this.serverConfig?.plantId || "",
    );
  ...
  async writeDashboardLiteStates(dashboardLite, updatedAt) {
    ...
    const recommendation = cards.find(
      (item) => item.label === "Recommendation",
    );
    await this.setStateAsync(
      "dashboard.lite",
      JSON.stringify(dashboardLite),
      true,
    );
    await this.setStateAsync("dashboard.lastUpdate", updatedAt, true);
    await this.setStateAsync(
      "dashboard.reason",
      dashboardLite.reason || "",
      true,
    );
    await this.setStateAsync(
      "dashboard.plan",
      JSON.stringify(dashboardLite.plan || []),
      true,
    );
    await this.setStateAsync(
      "dashboard.pattern",
      JSON.stringify(dashboardLite.pattern || {}),
      true,
    );
    await setNumericState(
      this,
      "dashboard.batteryCapacityKwh",
      cardValue("Battery capacity"),
    );
    await setNumericState(
      this,
    ...
    await setNumericState(
      ...
      cardValue("PV forecast", "PV forecast next 24h"),
    );
    await setNumericState(
      this,
  ...
  async applyBatteryControlFromDashboard(dashboardLite) {
    ...
    const assignment = batteryControlAssignment(
      this.config.datapointAssignments,
    );
    if (!assignment) {
      return;
  ...
  async pullControlCommands() {
    ...
    const commands =
      await this.apiClient.requestControlCommands(localConfigRevision);
    debug(
      this,
      "Control commands received",
      sanitizeForLog(commands, config.adapterToken),
    );
    if (this.shouldReloadServerConfig(commands, localConfigRevision)) {
      ...
      if (result.ok) {
        ...
        await this.pushAdapterMappings();
        if (
          Number.isFinite(loadedConfigRevision) &&
          loadedConfigRevision !== localConfigRevision
        ) {
          this.log.debug(
...
function adapterMappingFromAssignment(assignment = {}) {
  ...
  return {
    installationId:
      assignment.scope === "plant" ? String(assignment.scopeId || "") : null,
    telemetryField,
    statePath: stateId,
...
function dashboardLiteFromDecision(response = {}) {
  ...
  const system =
    decision.system && typeof decision.system === "object"
      ...
      : {};
  const inputSummary =
    decision.input_summary && typeof decision.input_summary === "object"
      ? decision.input_summary
      : {};
  const loadProfile =
    inputSummary.consumption?.load_profile &&
    typeof inputSummary.consumption.load_profile === "object"
      ? inputSummary.consumption.load_profile
      : {};
    ...
    pattern: {
      ...
      profiles: Array.isArray(loadProfile.profiles)
        ? loadProfile.profiles.map((profile) => ({
            ...
            days: profile.days ?? "-",
            averageKwh:
              nullableNumber(profile.average_wh) === null
                ? null
                : Number(profile.average_wh) / 1000,
            minKwh:
              nullableNumber(profile.min_wh) === null
                ? null
                : Number(profile.min_wh) / 1000,
            maxKwh:
              nullableNumber(profile.max_wh) === null
                ? null
                : Number(profile.max_wh) / 1000,
