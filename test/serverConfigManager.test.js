...
test("normalizes a valid server config", () => {
  ...
  assert.match(
    result.requiredDatapoints.gridPower.description,
    /instantaneous grid meter value/,
  );
});
...
test("builds household first and plant assignments from server config", () => {
  ...
  assert.deepEqual(
    assignments
      .filter((item) => item.scopeName === "Household")
      .map((item) => item.key),
  ...
  assert(
    assignments.some(
      (item) =>
        item.scopeName === "Household" &&
        item.key === "consumptionWh" &&
        item.stateId === "meter.0.import",
    ),
  );
  assert(
    assignments.some(
...
test("preserves selected source units when rebuilding assignments", () => {
  ...
  const exportMeter = assignments.find(
    (item) => item.key === "gridExportMeterWh",
  );
...
test("preserves battery control state and values when rebuilding assignments", () => {
  ...
  const control = assignments.find(
    (item) => item.mappingKey === "plant_a.batteryControlMode",
  );
...
test("maps plant grid charging permission to battery control assignment", () => {
  ...
  const allowed = assignments.find(
    (item) => item.mappingKey === "plant_allowed.batteryControlMode",
  );
  const denied = assignments.find(
    (item) => item.mappingKey === "plant_denied.batteryControlMode",
  );
...
test("defaults battery grid charging permission to allowed", () => {
  ...
  const assignments = buildDatapointAssignments(config, [], {});
  const control = assignments.find(
    (item) => item.mappingKey === "plant_default.batteryControlMode",
  );
...
test("uses readable labels for missing plant values", async () => {
  ...
  assert.deepEqual(payload.missing, [
    "gridPower",
    "6da81ee3-2e91-4780-b853-5093fbb9b6f1.batteryPower",
  ]);
